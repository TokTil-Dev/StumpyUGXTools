    ...
    static class ImageDDS
    {
        ...
        public static void LoadFromDisk(string filename, out uint texturehandle, out TextureTarget dimension)
        {
            ...
            catch (Exception e)
            {
                ...
                Program.editor.log.LogOut("Exception caught when attempting to load file " + filename + ".\n" + e, StumpyUGXTools.Editor.LogTab.LogType.Warning);
                // return; // failure
            }
            finally
            {
        ...
        public static void LoadFromDisk(byte[] file, out uint texturehandle, out TextureTarget dimension)
        {
            #region Prep data
            // invalidate whatever it was before
            dimension = (TextureTarget)0;
            ...
            catch (Exception e)
            {
                dimension = (TextureTarget)0;
                texturehandle = TextureLoaderParameters.OpenGLDefaultTexture;
                throw new ArgumentException("ERROR: Exception caught when attempting to load file from byte array." + ".\n" + e);
                // return; // failure
            }
            finally
            {
